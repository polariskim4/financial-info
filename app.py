...
# 1. Finviz 데이터 스크래핑 함수 (ROIC 매핑 로직 포함)
...
def get_finviz_data(ticker):
    ...
    try:
        ...
        # 핵심 해결: Finviz의 'ROI' 라벨을 'ROIC'로 매핑
...
# Error Fix: 고유 key를 추가하여 DuplicateElementId 에러를 방지합니다.
user_ticker = st.text_input(
    "비교할 종목 티커를 입력하세요:", 
    placeholder="예: AAPL, NVDA",
    key="unique_ticker_input_v1"
).upper()
...
if user_ticker:
    with st.spinner('데이터를 불러오는 중...'):
        ...
        if all_data:
            ...
            # Warning Fix: 최신 규격인 width="stretch"를 적용합니다.
            st.dataframe(
